...
class ForecastEntry:
    def __init__(
        self, time: datetime, total: float, low: float, mid: float, high: float
    ):
        self.time = time
        self.total = total
        self.low = low
...
class CloudForecast:
    ...
    def update(self) -> None:
        if (
            ...
            and (datetime.now(UTC) - self.last_update).total_seconds() > 6 * 60 * 60
        ):
            return
        ...
        start_time = datetime.fromtimestamp(
            hourly.Time() - response.UtcOffsetSeconds(), tz=UTC
        )
        forecast = []
    ...
    def get_clouds(self, time: datetime) -> Optional[ForecastEntry]:
        ...
        return ForecastEntry(
            time,
            self.forecast[before + 1].total * perc
            + self.forecast[before].total * (1 - perc),
            self.forecast[before + 1].low * perc
            + self.forecast[before].low * (1 - perc),
            self.forecast[before + 1].mid * perc
            + self.forecast[before].mid * (1 - perc),
            self.forecast[before + 1].high * perc
            + self.forecast[before].high * (1 - perc),
        )
